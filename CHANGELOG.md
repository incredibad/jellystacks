...
### Changed
- Sidebar: moved "Import from Jellyfin" from the Library section to the Collections section in the Operations accordion
...
### Fixed
- TheTVDB artwork images not loading: image URLs returned by the API are already fully-qualified, so the base URL prefix was being double-applied
...
### Changed
- Artwork override badge label changed from "Custom" to "Override"
...
### Changed
- Removed the Backdrops tab from the artwork picker modal — only posters are shown
...
### Fixed
- "Convert to Custom" moved into the settings dropdown alongside Verify, Remove from Jellyfin, and Delete Collection
