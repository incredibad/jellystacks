"""
Background scheduler for periodic managed-collection refresh.

Polls every 60 s; runs when the configured interval has elapsed.
The interval is read from the database on each tick so changes
in Settings take effect within a minute without a restart.
"""
import asyncio
import logging
from datetime import datetime, timedelta

logger = logging.getLogger("scheduler")

INTERVALS: dict[str, timedelta] = {
    "6h":     timedelta(hours=6),
    "12h":    timedelta(hours=12),
    "24h":    timedelta(hours=24),
    "weekly": timedelta(weeks=1),
}

_task: asyncio.Task | None = None


async def _run_refresh(db_factory) -> None:
    from refresh import refresh_all_managed
    from routers.settings import _get_settings_dict

    db = db_factory()
    try:
        s = _get_settings_dict(db)
        result = await refresh_all_managed(db, s.get("tmdb_api_key"), s.get("mdblist_api_key"))
        logger.info("Scheduled collection refresh complete: %s", result)
    except Exception:
        logger.exception("Scheduled collection refresh failed")
    finally:
        db.close()


async def _loop(db_factory) -> None:
    from routers.settings import _get_settings_dict

    last_run: datetime | None = None

    while True:
        await asyncio.sleep(60)

        db = db_factory()
        try:
            s = _get_settings_dict(db)
            interval_key = s.get("collection_refresh_interval", "disabled")
        finally:
            db.close()

        interval = INTERVALS.get(interval_key)
        if interval is None:
            continue

        now = datetime.utcnow()
        if last_run is None or (now - last_run) >= interval:
            logger.info("Running scheduled collection refresh (interval=%s)", interval_key)
            await _run_refresh(db_factory)
            last_run = now


def start(db_factory) -> None:
    global _task
    _task = asyncio.get_event_loop().create_task(_loop(db_factory))
    logger.info("Collection refresh scheduler started")


def stop() -> None:
    global _task
    if _task and not _task.done():
        _task.cancel()
    _task = None
